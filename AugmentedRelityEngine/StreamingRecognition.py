...
class Client:
    ...
    def __init__(self, language = 'en-US', context=[]):
    ...
    def listen(self):
        ...
        with self.record_audio(self.RATE, self.CHUNK) as buff:
            ...
            requests = self.request_stream(self, self._audio_data_generator(buff, overlap_buffer), self.RATE)
            ...
            try:
                while True:
                    speech_result = self.listen_print_loop(recognize_stream, buff)
    ...
    def request_stream(self, data_stream, rate, interim_results=True):
        ...
        cont = cloud_speech_pb2.SpeechContext(phrases=self.context)
        recognition_config = cloud_speech_pb2.RecognitionConfig(
            ...
            language_code=self.language,  # a BCP-47 language tag
            speech_context=cont,
        )
